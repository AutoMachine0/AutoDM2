import torch
from search_space.mlp_conv import MLP
...
class ConvPool(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if conv_name == "GCNConv":
            ...
            nn = Sequential(Linear(input_dim, output_dim),
                            ReLU(),
                            Linear(output_dim, output_dim))
            self.conv_operation = GINConv(nn)
        elif conv_name == "MLPConv":
            self.conv_operation = MLP(input_dim, output_dim)
        else:
